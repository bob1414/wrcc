...
NS_LOG_COMPONENT_DEFINE("RDMA_HW");
...
TypeId RdmaHw::GetTypeId (void)
{
	static TypeId tid = TypeId ("ns3::RdmaHw")
		...
		.AddAttribute("PintSmplThresh",
				...
				MakeUintegerChecker<uint32_t>())
		.AddAttribute("SELF_RISE_INTERVAL",
				"SELF RISE INTERVAL",
				UintegerValue(55000),
				MakeUintegerAccessor(&RdmaHw::Inter_RateIncINTERVAL),
				MakeUintegerChecker<uint32_t>())
		;
	return tid;
}
...
void RdmaHw::AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish){
	...
	if (m_cc_mode == 1){
		...
		qp->wrcc.firstbdp = true;
		qp->wrcc.m_curRate = m_bps;
		qp->wrcc.m_max_rate = m_bps;
		qp->wrcc.m_wrcclastUpdateSeq = 0;
		//Default RxDomain is different from domain_id, Avoid Different Domain Increment
		qp->wrcc.rxdomain = 999;
		// qp->wrcc.rxdomain = m_node->m_domain_id;
		qp->wrcc.EstRTT = 0;
		ScheduleInterRateIncrease(qp);
	}else{
		NS_ASSERT_MSG(false, "Ubkown CC Mode !\n");
	}
...
int RdmaHw::ReceiveUdp(Ptr<Packet> p, CustomHeader &ch){
	...
	/*WRCC Receiver Static*/
	if(m_cc_mode == 15){
		uint32_t nic_idx = GetNicIdxOfRxQp(rxQp);		
		DataRate tarrate = m_nic[nic_idx].dev->GetDataRate();
		
		//Pkt Info
		uint32_t fairate = ch.udp.ih.wrcc.Fairate;//Mbps
		std::string temprate = std::to_string(fairate)+"Mbps";
		DataRate frate = std::min(DataRate(temprate),tarrate);

		ch.udp.ih.wrcc.domainid = m_node->m_domain_id;
		/*Marked Special INT head, Can't Modify INT Rate;1000 is Normal ACK/NAK; 1001 is FCNP*/
		ch.udp.ih.wrcc.firstbdp = 1000;
		ch.udp.ih.wrcc.Fairate = frate.GetBitRate()/1000000;
	}
...
WRCCTrig:
	if (m_cc_mode == 15){//WRCC
		if(fbdp == 1001){
			HandleInterRatePkt(qp, p, ch);//Switch Fast FairRate Pkt Handler, can not triger more packets to send
			return 0;
		}
		else
			HandleGeneACK(qp, p, ch);
	}
...
void RdmaHw::QpComplete(Ptr<RdmaQueuePair> qp){
	...
	if (m_cc_mode == 1){
		...
		Simulator::Cancel(qp->wrcc.m_rateIncrease);
	}
...
Ptr<Packet> RdmaHw::GetNxtPacket(Ptr<RdmaQueuePair> qp){
	...
	seqTs.SetPG (qp->m_pg);
	if(m_cc_mode == 15){
		/*WRCC Pkt head Modify*/
		uint64_t wts = Simulator::Now().GetTimeStep();
		uint32_t ertt = qp->wrcc.EstRTT;
		seqTs.SetWRCC(qp->wrcc.firstbdp,qp->wrcc.m_curRate.GetBitRate()/1000000,this->m_node->m_domain_id,wts,ertt);
	}
...
void RdmaHw::UpdateRateTimely(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool us){
	...
	bool print = !us;
	uint64_t minRtt = m_tmly_minRtt, lowRTT = m_tmly_TLow, highRTT = m_tmly_THigh;
	...
	if (qp->tmly.m_lastUpdateSeq != 0){ // not first RTT
		...
		double gradient = rtt_diff / minRtt;
		bool inc = false;
		double c = 0;
		#if PRINT_LOG
		if (print)
		...
		if (rtt < lowRTT){
			...
			c = 1 - m_tmly_beta * (1 - (double)highRTT / rtt);
			inc = false;
		}else if (gradient <= 0){
			inc = true;
		}else{
...
/*********************
 * WRCC   Wan RDMA Congestion Control CC-Mode 15
 ********************/
void RdmaHw::ScheduleInterRateIncrease(Ptr<RdmaQueuePair> qp){
	qp->wrcc.m_rateIncrease = Simulator::Schedule(NanoSeconds(Inter_RateIncINTERVAL),&RdmaHw::HandleInterRateIncrease,this,qp);
}

void RdmaHw::HandleInterRateIncrease(Ptr<RdmaQueuePair> qp){
	NS_LOG_FUNCTION(this<<this->m_node->GetId())

	double inc_alf = 2.0;

	DataRate m_rate = qp->wrcc.m_curRate * inc_alf;
	if(m_rate > qp->wrcc.m_max_rate){
		m_rate = qp->wrcc.m_max_rate;
	}
	qp->wrcc.m_curRate = m_rate;
	qp->wrcc.m_curRate = std::min(qp->wrcc.m_curRate * inc_alf,qp->wrcc.m_max_rate);

	ScheduleInterRateIncrease(qp);
}

void RdmaHw::HandleInterRatePkt(Ptr<RdmaQueuePair> qp,Ptr<Packet> p, CustomHeader &ch){
	NS_LOG_FUNCTION(this<<this->m_node->GetId());
	//INT Data
	uint16_t domainid = ch.ack.ih.wrcc.domainid;
	uint16_t fbdp = ch.ack.ih.wrcc.firstbdp;
	uint32_t fairate = ch.ack.ih.wrcc.Fairate;//Mbps

	uint32_t adv_psn = ch.ack.seq;

	//Data Process
	std::string temprate = std::to_string(fairate)+"Mbps";
	DataRate frate = DataRate(temprate);

	if(qp->wrcc.m_curRate > frate){
		qp->wrcc.m_curRate = frate;
		ChangeRate(qp,qp->wrcc.m_curRate);
		Simulator::Cancel(qp->wrcc.m_rateIncrease);
		ScheduleInterRateIncrease(qp);

		/*Update the Advanced ACK PSN*/
		qp->wrcc.m_wrcclastUpdateSeq = adv_psn;
	}
}

void RdmaHw::HandleGeneACK(Ptr<RdmaQueuePair> qp,Ptr<Packet> p, CustomHeader &ch){
	NS_LOG_FUNCTION(this<<this->m_node->GetId());
	//INT Data
	uint16_t remote_domainid = ch.ack.ih.wrcc.domainid;
	uint16_t fbdp = ch.ack.ih.wrcc.firstbdp;
	uint32_t fairate = ch.ack.ih.wrcc.Fairate;//Mbps
	uint64_t rtt = Simulator::Now().GetTimeStep() - ch.ack.ih.wrcc.wts;
	uint64_t temprtt;
	...
	if (qp->wrcc.EstRTT == 0){
		...
		temprtt = (1 - m_tmly_alpha)*qp->wrcc.EstRTT + m_tmly_alpha * (rtt/1000);
	}
	qp->wrcc.EstRTT = temprtt;

	//Local Data
	uint16_t local_domain = this->m_node->m_domain_id;
	uint32_t ack_seq = ch.ack.seq;
	DataRate m_curate = qp->wrcc.m_curRate;
	uint32_t adv_psn = qp->wrcc.m_wrcclastUpdateSeq;
	...
	if(frate < m_curate || ack_seq > adv_psn){
		tarrate = frate;
		ChangeRate(qp,tarrate);
		/*Update the Advanced ACK PSN*/
		qp->wrcc.m_wrcclastUpdateSeq = ack_seq;

	}

	//Domain ID Update
	if(qp->wrcc.rxdomain != remote_domainid){
		qp->wrcc.rxdomain = remote_domainid;
	}
	if(local_domain == remote_domainid){
		qp->wrcc.firstbdp = false;
	}else{
		qp->wrcc.firstbdp = true;
	}
}
