...
public:
	...
	/*WRCC Switch*/
	bool m_dynamicth;//PFC switch dynamic threshold, En Shared Mem
	bool m_customecn;//Custom ECN per Port, ECN
	...
	std::unordered_map<uint32_t, uint32_t> pfcth;//PFC switch static threshold <port,pfcth>  Xoff==Xon
	std::unordered_map<uint32_t, ecnconfig> customecn;

	void ConfigPortPFC(uint32_t dstid,uint32_t Xoff);
	void ConfigDyPFCEn(uint32_t PFCEN);
	void ConfigCustomECN_EN(uint32_t CustomECNEN);
	void ConfigCustomECN(uint32_t port,	uint32_t kmin,uint32_t kmax,double pmax);
