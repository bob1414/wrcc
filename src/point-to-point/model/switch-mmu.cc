...
namespace ns3 {
	...
	// }
	uint32_t SwitchMmu::GetPfcThreshold(uint32_t port){
		//Switch Port PFC threshold
		std::unordered_map<uint32_t, uint32_t>::iterator it = pfcth.find(port);
		if(!m_dynamicth){
			NS_ASSERT_MSG(it != pfcth.end(),"must set static PFC for each Port in Switch "<<node_id<<" Porid "<<port);
			return it->second;
		}
		else
		{
			NS_ASSERT_MSG(buffer_size > (total_hdrm+total_rsrv+shared_used_bytes),"SwitchBuff Size Config Error! buffer_size:"<<buffer_size<<" total_hdrm:"<<total_hdrm<<" rsrv:"<<total_rsrv<<" used:"<<shared_used_bytes<<" port_headroom:"<<headroom[port]<<std::endl);
			//switch dynamic threshold
			return (buffer_size - total_hdrm - total_rsrv - shared_used_bytes) >> pfc_a_shift[port];
		}
	}
	...
	bool SwitchMmu::ShouldSendCN(uint32_t ifindex, uint32_t qIndex){
		if(m_customecn){
			auto it = customecn.find(ifindex);
			if(it != customecn.end() ){
				if(it->second.qIndex == qIndex){
					if (egress_bytes[ifindex][qIndex] > it->second.kmax)
						return true;
					if (egress_bytes[ifindex][qIndex] > it->second.kmin){
						double p = it->second.pmax * double(egress_bytes[ifindex][qIndex] - it->second.kmin) / (it->second.kmax - it->second.kmin);
						if (UniformVariable(0, 1).GetValue() < p)
							return true;
					}
				}
	...
	/*WRCC Function*/
	void SwitchMmu::ConfigDyPFCEn(uint32_t PFCEN){
		if(PFCEN == 0)
			m_dynamicth = false;
		else
			m_dynamicth = true;
	}
	void SwitchMmu::ConfigPortPFC(uint32_t dstid,uint32_t Xoff){
		pfcth[dstid] = Xoff;
		// std::cout<<dstid<<" "<<Xoff<<std::endl;
	}
	void SwitchMmu::ConfigCustomECN_EN(uint32_t CustomECNEN){
		if(CustomECNEN > 0)
			m_customecn = true;
		else
			m_customecn = false;
	}
	void SwitchMmu::ConfigCustomECN(uint32_t port,	uint32_t kmin,uint32_t kmax,double pmax){
		uint32_t qIndex = 3;
		ecnconfig temp;
		temp.qIndex = qIndex;
		temp.kmin = kmin;
		temp.kmax = kmax;
		temp.pmax = pmax;
		auto it = customecn.find(port);
		if(it == customecn.end()){
			customecn[port] = temp;
		}
		else{
			std::cout<<"Warning : Reconfig Port"<<port<<" Custom ECN"<<"\n";
			it->second.qIndex = qIndex;
			it->second.pmax = pmax;
			it->second.kmin = kmin;
			it->second.kmax = kmax;
		}
