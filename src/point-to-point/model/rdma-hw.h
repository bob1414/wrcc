...
public:
	 ...
	 * WRCC, CC_Mode 15
	 ********************/
	uint32_t Inter_RateIncINTERVAL; /*Inter-Flow Quick Rate Increase*/
	
	void ScheduleInterRateIncrease(Ptr<RdmaQueuePair> qp);
	void HandleInterRateIncrease(Ptr<RdmaQueuePair> qp);
	void HandleInterRatePkt(Ptr<RdmaQueuePair> qp,Ptr<Packet> p, CustomHeader &ch);
	void HandleGeneACK(Ptr<RdmaQueuePair> qp,Ptr<Packet> p, CustomHeader &ch);

	//NP Receiver, Simple Max Bandwidth Static,Debug Mod
	EventId m_ReceiverFLowStatic;
	bool Is_ScheduleRecStat;
	std::unordered_map<uint64_t, uint32_t> ReceiverStatic;//RXQP map to m_rxQpMap, Store last Static Period's Pkt Num
	void ScheduleReceiverStat();
	void ReceiverNumStatic();
};
