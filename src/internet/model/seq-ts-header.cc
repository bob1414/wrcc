...
void SeqTsHeader::SetWRCC(bool firstbdp,uint32_t rate,uint16_t domain,uint64_t wtimestamp,uint32_t eRTT){
  if (IntHeader::mode == IntHeader::WRCC){
    ih.wrcc.Fairate = rate;
    ih.wrcc.domainid = (uint16_t)domain;
    ih.wrcc.wts = wtimestamp;
    ih.wrcc.RTTest = eRTT;
		if(firstbdp)
			ih.wrcc.firstbdp = 1;
		else
