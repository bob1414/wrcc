...
void IntHeader::Serialize (Buffer::Iterator start) const{
	...
	if (mode == NORMAL){
		...
		i.WriteU16(wrcc.firstbdp);
		i.WriteU16(wrcc.domainid);
		i.WriteU32(wrcc.Fairate);
		i.WriteU32(wrcc.RTTest);
		i.WriteU64(wrcc.wts);
	}
}
...
uint32_t IntHeader::Deserialize (Buffer::Iterator start){
	...
	if (mode == NORMAL){
		...
		wrcc.firstbdp = i.ReadU16();
		wrcc.domainid = i.ReadU16();
		wrcc.Fairate = i.ReadU32();
		wrcc.RTTest = i.ReadU32();
		wrcc.wts = i.ReadU64();
	}
